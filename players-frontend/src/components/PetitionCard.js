...
export default function PetitionCard({ petition }) {
  ...
  return (
    <article className="card-glass p-5 card-hover">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-zinc-500">{item.game_name || 'Unknown Game'}</p>
          <Link to={`/petitions/${item.slug || item.id}`} className="block mt-2">
            <h3 className="font-heading text-2xl font-bold text-zinc-100 leading-tight hover:text-white">
              {item.title || 'Untitled petition'}
            </h3>
          </Link>
        </div>
        <Tag tone={item.eligible_for_studio_push ? 'success' : 'neutral'}>
          {item.eligible_for_studio_push ? 'Milestone reached' : 'Growing'}
        </Tag>
      </div>

      <p className="mt-3 text-sm text-zinc-300 leading-relaxed">{item.summary || 'No summary provided.'}</p>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 text-xs text-zinc-500">
        ...
        <Link to={`/petitions/${item.slug || item.id}`} className="text-zinc-300 hover:text-white">View petition</Link>
      </div>
    </article>
  );
}
